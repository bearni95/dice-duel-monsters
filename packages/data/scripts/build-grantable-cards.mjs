...
// Marker every allow-list migration carries, so the next run can tell whether the
// set actually changed without having to parse SQL back into ids.
const FINGERPRINT_PREFIX = 'allow-list:';
...
if (migrationsFlag !== -1) {
	...
	// Every migration that states the allow-list carries this line, so "has the set
	// changed?" is a string comparison against the last one rather than a diff of
	// two differently-shaped SQL bodies.
	const fingerprint = `-- ${FINGERPRINT_PREFIX} ${grantable.length}:${createHash('sha256')
		.update(grantable.join(','))
		.digest('hex')
		.slice(0, 16)}`;

	// Matches both the initial `_available_cards` migration and every later
	// `_sync_available_cards` one; the newest of them holds the current set.
	const previous = migrations.filter((f) => f.endsWith('_available_cards.sql')).at(-1);

	if (previous && readFileSync(join(migrationsDir, previous), 'utf8').includes(fingerprint)) {
		console.log(
		...
		// existing migration. A UTC clock alone doesn't guarantee that — the stamps
		// already in the tree were not all written by this machine's clock — so step
		// past the newest one when now isn't already later.
		const now = new Date().toISOString().replace(/[-:T]/g, '').slice(0, 14);
		const newest = migrations.length ? migrations[migrations.length - 1].slice(0, 14) : '';
		const stamp = now > newest ? now : String(BigInt(newest) + 1n);
